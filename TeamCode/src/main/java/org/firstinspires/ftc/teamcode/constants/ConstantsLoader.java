...
import org.firstinspires.ftc.teamcode.constants.exceptions.EnumLoadingException;
...
public final class ConstantsLoader {
    ...
    @NonNull
    private List<File> loadConstantsFiles() {
        File constantsDirectory = new File(CONSTANTS_FOLDER_PATH);

        if (constantsDirectory.isFile()) {
            debugger.addMessage(
                    "Failed To Load Constants File Directory"
    ...
    public void load() {
        for (File file : loadConstantsFiles()) {
            ...
            try {
                ...
                debugger.addMessage(
                        "Skipped Class " + fileName
                        + "\nReason: No Constants File Associated With Class Name"
                );
            }
        }
    ...
    private void loadClass(@NonNull Class<?> clazz, @NonNull String fileName) {
        ...
        try {
            properties.load(new FileInputStream(CONSTANTS_FOLDER_PATH + fileName));
            for (Field field : clazz.getFields()) {
                try {
                    loadField(field, properties);
                } catch (MalformedPropertyException ignored) {}
            }
    ...
    private void loadField(
        ...
        try {
            switch (SupportedType.fieldToType(field)) {
                ...
                case UNSUPPORTED:
                    debugger.addMessage(
                            "Failed To Load Field " + fieldName
    ...
    @NonNull
    private String stripFileExtension(@NonNull String fileName) {
        return fileName.substring(0, fileName.lastIndexOf('.'));
    }

    @Nullable private Object loadEnum(
            @NonNull String key,
            @NonNull Properties properties,
            @NonNull Class<?> enumClass
    ) throws EnumLoadingException{
        if (!enumClass.isEnum()) {
           throw new EnumLoadingException(
                   "Failed To Load Enum " + enumClass.getSimpleName()
                   + "\nReason: " + enumClass.getSimpleName() + " is not an enum"
           );
        }

        String enumString = properties.getProperty(key).toLowerCase().replace("_", "");

        for (Object varient : enumClass.getEnumConstants()) {
            if (varient.toString().toLowerCase().equals(enumString)) {
                return varient;
            }
        }
    ...
    @NonNull
    private Servo.Direction loadServoDirection(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private DcMotorSimple.Direction loadMotorDirection(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private DcMotor.ZeroPowerBehavior loadZeroPowerBehaviour(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private DcMotor.RunMode loadRunMode(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private UsbFacingDirection loadUsbFacingDirection(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private LogoFacingDirection loadLogoFacingDirection(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private SparkFunOTOS.Pose2D loadSparkFunPose2D(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private AngleUnit loadAngleUnit(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private DistanceUnit loadDistanceUnit(
            @NonNull String key,
            @NonNull Properties properties
    ) throws MalformedPropertyException {
    ...
    @NonNull
    private double[] parseThreePartValue(
